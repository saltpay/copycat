...
1. **Project Configuration (`projects.yaml`)**
   - YAML-based repository manifest
   - Supports metadata: Slack channels, CDE flags
   - Organization hardcoded as "saltpay"
...
3. **PR Titles**:
   - CDE projects: `JIRA-123 - Description`
...
### CDE (Continuous Delivery Environment) Projects

- **Mandatory Jira ticket**: PR title must include Jira reference
- **Format**: `JIRA-123 - Description`
- **Detection**: Based on `in_cde: true` in `projects.yaml`
- **Validation**: Tool prompts for Jira ticket if CDE projects selected
...
6. **Document business rules**: CDE flags, Jira tickets, etc.
